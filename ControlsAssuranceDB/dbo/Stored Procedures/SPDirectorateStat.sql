...
CREATE PROCEDURE [dbo].[SPDirectorateStat] @PeriodId int
AS
...
(Count(Team.ID) * (select Count(ID) from DefElement where PeriodId=@PeriodId) ) as 'TotalElements' from Directorate
