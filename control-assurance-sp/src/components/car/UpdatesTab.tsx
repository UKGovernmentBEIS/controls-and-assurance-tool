...
import { IGIAARecommendation, IDefElement, IElement, Element, IUserHelp, IFForm, ElementStatus, ResponsesA } from '../../types';
...
export default class UpdatesTab extends React.Component<IUpdatesTabProps, IUpdatesTabState> {
                ...
                <React.Fragment>
                    ...
                    {choiceVal && choiceVal === "Yes" &&
                        <CrTextField
                            ...
                            maxLength={500}
                            charCounter={true}
                            onChanged={(ev, newValue) => this.changeTextField(newValue, textBoxFormDataProperty)}
                            value={textBoxVal}
                        />
                    }
                </React.Fragment>
                ...
                <div className={styles.flexContainerSectionBQuestionDetail}>
                    ...
                    <div className={styles.sectionBQuestionDetailCol2}>
                        ...
                        <CrTextField
                            ...
                            maxLength={500}
                            charCounter={true}
                            onChanged={(ev, newValue) => this.changeTextField(newValue, responseB1TextformDataProperty)}
    ...
    private loadDefElement = (): void => {
        this.defElementService.read(this.state.DefElementId).then((df: IDefElement): IDefElement => {
    ...
    private loadEntityUpdate = (): void => {
        const formId: number = this.props.formId;
        const defElementId: number = this.state.DefElementId;
        this.elementService.readElement(formId, defElementId).then((eu: IElement[]) => {
            if (eu.length > 0) {
                const formData = eu[0];
